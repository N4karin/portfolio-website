...
const SkillCard: React.FC<SkillCardProps> = ({ path, name, viewbox }) => {
    ...
    return (
        <div className="relative flex flex-col items-center group">
            ...
            />
            <div
                className={`opacity-0 group-hover:opacity-100 transition-transform duration-300 text-indigo-500 transform group-hover:-translate-y-4 
                    ${name.length > 11 ? 'text-sm' : 'text-base'
                    }`}
            >
                {name}
            </div>
